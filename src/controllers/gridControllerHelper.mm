...
void gridControllerHelper::drawLocation(int location)
{
	for(int i=0;i<6;i++){
		if(i==location){
			this->drawRectTexture(245+(i%3)*25, 401+(i/3)*38, 23, 35, 25, 647, 0);
		}else{
			this->drawRectTexture(245+(i%3)*25, 401+(i/3)*38, 23, 35, 0, 647, 0);
		}
	}
}
